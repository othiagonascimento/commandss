...
export default function EditTenant() {
  ...
  const [formData, setFormData] = useState<TenantFormData>({
    ...
    implementation_paid_externally: false,
    // Monthly fee default
  ...
  useEffect(() => {
    if (tenant) {
      const tenantData = tenant as TenantDetail & {
        price_per_user?: number;
        contracted_users?: number;
        channel_price?: number;
        extra_channels?: number;
      ...
      setFormData({
        ...
        subdomain: tenantData.slug || tenantData.subdomain || '',
        ...
        channel_price: tenantData.channel_price ?? 0,
        extra_channels: tenantData.extra_channels ?? 0,
        discount_type: tenantData.discount_type || '',
        discount_value: tenantData.discount_value ?? 0,
        // Implementation
        implementation_fee: tenantData.implementation_fee ?? 0,
        implementation_status: tenantData.implementation_status || 'pending',
        implementation_paid_externally: tenantData.implementation_paid_externally || false,
        // Monthly fee
        has_monthly_fee: tenantData.has_monthly_fee ?? true,
      });
    }
  }, [tenant]);

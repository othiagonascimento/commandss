...
-- Fix handle_new_user: 
-- 1) Fill 'subdomain' column (NOT NULL) when creating tenant
-- 2) Fix ON CONFLICT to use (user_id, tenant_id) composite key
...
BEGIN
  ...
  IF v_tenant_id IS NULL THEN
    ...
    v_slug := COALESCE(NEW.raw_user_meta_data ->> 'company_slug', 'tenant-' || SUBSTRING(gen_random_uuid()::text, 1, 8));
    
    INSERT INTO public.tenants (name, subdomain, subdomain_slug, is_master)
    VALUES (
  ...
  INSERT INTO public.user_usage (user_id, tenant_id) VALUES (NEW.id, v_tenant_id) ON CONFLICT (user_id, tenant_id) DO NOTHING;
  INSERT INTO public.user_limits (user_id, tenant_id) VALUES (NEW.id, v_tenant_id) ON CONFLICT (user_id, tenant_id) DO NOTHING;
